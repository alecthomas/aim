...
/// dbmate format: timestamped single files with `-- migrate:up` / `-- migrate:down` directives.
///
/// Filename: `YYYYMMDDHHMMSS_description.sql`
pub struct Dbmate;

impl MigrationFormat for Dbmate {
...
/// Parse `YYYYMMDDHHMMSS_description.sql`.
...
/// Split file content on `-- migrate:up` and `-- migrate:down` markers.
