...
impl MigrationFormat for GolangMigrate {
    fn list(&self, dir: &Path) -> Result<Vec<Migration>, Error> {
        ...
        for (seq, description) in &entries {
            let up_path = dir.join(format!("{seq:06}_{description}.up.sql"));
            let down_path = dir.join(format!("{seq:06}_{description}.down.sql"));
...
/// Parse a filename like `000001_add_users.up.sql` into (sequence, description).
...
mod tests {
    ...
    fn test_parse_filename() {
        assert_eq!(
            ...
            Some((1, "initial".to_owned()))
        );
        assert_eq!(
            parse_filename("000042_add_email.down.sql"),
            Some((42, "add_email".to_owned()))
        );
